...
### Страница для обычного HTTPS

В Swarm override для сервиса `https` включён `SNI_CERT_STATIC_1=true` рядом
с `SNI_CERT_1`. Caddy отдаёт публичную статическую страницу из
`services/https/files/certificate-site/index.html` на внутреннем HTTPS-порту 444.
Входной SNI-маршрут по-прежнему ведёт в telemt: корректный секрет включает
MTProxy, обычный браузер попадает через masking на страницу Caddy.
Authelia для этой публичной страницы не используется.

При изменении страницы или генератора пересоберите и опубликуйте образ `https`
(для этой версии `nmisha/antizapret-vpn-https:6.7.9-5`), затем разверните стек
через `sr_swarm_start.sh`. Образ telemt и его конфиг менять не требуется.
После деплоя проверьте домен маскировки браузером и подключение Telegram.
Обычная страница не гарантирует нераспознаваемость прокси.
