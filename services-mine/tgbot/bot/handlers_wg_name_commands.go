...
func RegisterWgNameCommands(r *Router) {
	// Admin-only name-based actions with disambiguation
	r.Handle("/wgenable", handleWgNameEnable, RequireRole(RoleAdmin, "Недостаточно прав. Нужна роль Admin."), RequireNonEmptyArg("Формат: /wgenable <profile_name_or_prefix>"))
	r.Handle("/wgdisable", handleWgNameDisable, RequireRole(RoleAdmin, "Недостаточно прав. Нужна роль Admin."), RequireNonEmptyArg("Формат: /wgdisable <profile_name_or_prefix>"))
	r.Handle("/wgconf", handleWgNameConf, RequireRole(RoleAdmin, "Недостаточно прав. Нужна роль Admin."), RequireNonEmptyArg("Формат: /wgconf <profile_name_or_prefix>"))
	r.Handle("/wgqr", handleWgNameQR, RequireRole(RoleAdmin, "Недостаточно прав. Нужна роль Admin."), RequireNonEmptyArg("Формат: /wgqr <profile_name_or_prefix>"))
	r.Handle("/wgdel", handleWgNameDel, RequireRole(RoleAdmin, "Недостаточно прав. Нужна роль Admin."), RequireNonEmptyArg("Формат: /wgdel <profile_name_or_prefix>"))
	r.Handle("/wgrename", handleWgNameRename, RequireRole(RoleAdmin, "Недостаточно прав. Нужна роль Admin."), RequireNonEmptyArg("Формат: /wgrename <old_name_or_prefix> <new_name>"))
}
