...
func handleWgNameCallback(ctx *Ctx, data string) {
	if !ctx.IsPrivate {
	...
	// format: wgn:<action>:<peerID>[:<extra>]
	rest := strings.TrimPrefix(data, wgNameCbPrefix)
	...
	performWgPeerAction(ctx, client, action, peerID, extra)
}
